<div class="sample-element">
    <img src="/src/Assets/images/logo.png" alt="Cresto logo" width="128" height="128">

    <div style="max-width: 540px; padding: 4px;">
        <h1>
            Cresto
        </h1>

        <p>
            Cresto is a lightweight, straightforward PHP framework built to simplify web development. It’s designed around a clean folder structure, allowing you to easily manage pages.
        </p>
    </div>

    <a class="button" style="margin-top: 20px;" href="https://github.com/pavelkalas/cresto.git" target="_blank">
        <img src="/src/Assets/images/icons/github.png" alt="GitHub icon">Cresto on GitHub
    </a>
</div>
